var flg = "0"
var bootstrap // Declare the bootstrap variable
...
function menu_click() {
  ...
  for (var j = 0; j < menuItems.length; j++) {
    menuItems[j].removeEventListener("click", () => {})
  }

  // Hide submenu items (when menu link not active then submenu hide)
  var submenuItems = document.querySelectorAll(".pc-navbar li:not(.pc-trigger) .pc-submenu")
  ...
  for (var i = 0; i < pc_link_click.length; i++) {
    pc_link_click[i].addEventListener("click", (event) => {
  ...
  if (document.querySelector(".navbar-content")) {
    // Safely access SimpleBar from window
    if (typeof window.SimpleBar !== "undefined") {
      new window.SimpleBar(document.querySelector(".navbar-content"))
    } else {
      console.warn("SimpleBar not loaded, cannot initialize navbar content scrollbar.")
    }
  }
  ...
  for (var m = 0; m < pc_sub_link_click.length; m++) {
    pc_sub_link_click[m].addEventListener("click", (event) => {
      var targetElement = event.target
      if (targetElement.tagName == "SPAN") {
        targetElement = targetElement.parentNode
      }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (typeof window.feather !== "undefined") {
    ...
    console.warn("Feather icons not loaded, cannot replace icons.")
  }
  // feather icon end
  ...
  if (mobile_collapse_over) {
    mobile_collapse_over.addEventListener("click", () => {
  ...
  if (document.querySelector(".header-notification-scroll")) {
    if (typeof window.SimpleBar !== "undefined") {
      new window.SimpleBar(document.querySelector(".header-notification-scroll"))
    }
  }

  if (document.querySelector(".profile-notification-scroll")) {
    if (typeof window.SimpleBar !== "undefined") {
      new window.SimpleBar(document.querySelector(".profile-notification-scroll"))
    }
  }
  ...
  if (document.querySelector(".component-list-card .scroll-div")) {
    if (typeof window.SimpleBar !== "undefined") {
      new window.SimpleBar(document.querySelector(".component-list-card .scroll-div"))
    }
  }
  ...
  if (document.querySelector(".trig-drp-search")) {
    ...
    // Check if Bootstrap's dropdown is available before adding listener
    if (window.bootstrap && window.bootstrap.Dropdown) {
      // No need to re-initialize if already initialized by Bootstrap's auto-init
      search_drp.addEventListener("shown.bs.dropdown", (event) => {
        document.querySelector(".drp-search input").focus()
      })
    } else {
      console.warn("Bootstrap dropdown not loaded, search input focus might not work as expected.")
    }
  }
...
function setLayout() {
  ...
  // Ensure main_layout_change is globally accessible or defined before this call
  if (window.main_layout_change) {
    window.main_layout_change(layout)
  } else {
    console.warn("main_layout_change function not found.")
  }
    ...
    if (layout === "horizontal") {
      ...
      if (document.querySelector(".pc-sidebar .m-header .logo-lg")) {
        document
          ...
          .setAttribute("src", "../assets/images/LOGO_RDP-removebg-preview.png")
      }
  ...
  if (layout === null) {
    if (window.main_layout_change) {
      window.main_layout_change("vertical")
    }
...
// GLOBAL ERROR HANDLER: Added to catch any unhandled JavaScript errors
window.addEventListener("error", (event) => {
  console.error("Unhandled JavaScript Error:", event.message, "at", event.filename, ":", event.lineno)
  // As a fallback, try to hide the loader if an error occurs
  var loader = document.querySelector(".loader-bg")
  if (loader) {
    loader.style.display = "none" // Force hide the loader
    loader.style.opacity = "0" // Ensure it's transparent
    loader.remove() // Remove immediately
    console.log("script.js: Loader forcibly hidden via onerror.")
  }
  // Also show the global error display if available
  const globalErrorDisplay = document.getElementById("globalErrorDisplay")
  if (globalErrorDisplay) {
    globalErrorDisplay.classList.remove("hidden", "opacity-0")
    globalErrorDisplay.style.opacity = "1"
  }
})

// COMBINED window.load listener for loader removal, menu active state, and auth logo
window.addEventListener("load", () => {
  console.log("script.js: window.load fired.")

  // Remove pre-loader after page load with a fade-out effect
  var loader = document.querySelector(".loader-bg")
  if (loader) {
    console.log("script.js: Hiding loader via window.load.")
    ...
    setTimeout(() => {
      loader.remove()
      console.log("script.js: Loader removed via window.load.")
    }, 500) // Duration matches the CSS transition duration
  } else {
    console.log("script.js: Loader element not found on load.")
  }
  ...
  document.querySelectorAll(".auth-main.v2 .img-brand").forEach((img) => {
    img.setAttribute("src", "../assets/images/LOGO_RDP-removebg-preview.png")
  })
})
...
window.addEventListener("unhandledrejection", (event) => {
  ...
  // Show global error display if available
...
function hideLoader() {
  ...
  if (loader) {
    console.log("Main script: Hiding loader.")
    loader.style.opacity = "0"
    setTimeout(() => {
...
setTimeout(() => {
  console.log("Main script: Fallback loader hide after timeout.")
  hideLoader()
}, 3000)
